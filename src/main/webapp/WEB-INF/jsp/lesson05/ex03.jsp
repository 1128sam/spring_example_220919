...
<body>
	...
	<h1>Type</h1>
	숫자: <fmt:formatNumber value="${number}" type="number"/><br> <%-- type은 number로 자동설정되어 있음 --%>
	...
	<h1>var로 변수에 저장</h1>
	<fmt:formatNumber var="dollar" value="${number}" type="currency" currencySymbol="$" /><%--var가 있으면 변수에 저장만 함, print는 안함 --%>
	달러 EL로 출력: ${dollar}
	...
	3.141592(#.####) : <fmt:formatNumber value="3.141592" pattern="#.####" /><br><%--반올림됨--%>

	<hr>

	<h1>Date 객체를 String 형태로 출력 => fmt:formatDate</h1>
	<fmt:formatDate var="pattern1" value="${today}" pattern="yyyy년 M월 d일 HH시 mm분 ss초" />
